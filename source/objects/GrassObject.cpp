...
void GrassObject::onEvent(u8 event) {
	if(event == Map::EventType::GrassCutted) {
		...
		if(rand() % 10 == 0) {
			Map::currentMap->addCollectable<RupeeCollectable>(m_x, m_y, 1);
		}
		else if(rand() % 30 == 14) {
			Map::currentMap->addCollectable<HeartCollectable>(m_x, m_y);
		}
		else if(rand() % 50 == 22) {
			Map::currentMap->addCollectable<RupeeCollectable>(m_x, m_y, 5);
		}
		else if(rand() % 500 == 333) {
			Map::currentMap->addCollectable<RupeeCollectable>(m_x, m_y, 30);
		}
